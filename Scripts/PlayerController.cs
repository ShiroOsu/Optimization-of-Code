...
public class PlayerController : MonoBehaviour
{
	...
	private void Update()
	{
		...
		{
			LevelManager.instance.LevelScore += 1000;
		}
	...
	private void OnCollisionEnter2D(Collision2D collision)
	{
		...
		if (avg > 2.5f)
		{
			if (collision.gameObject.tag != "LaserShot" && collision.gameObject.tag != "Bullet" && collision.gameObject.tag != "Cargo" && collision.gameObject.tag != "Bounds" && collision.gameObject.tag != "Debris")
			{
	...
	private void PlayerMovement()
	{
		...
		if (thrustInput > 0 || isBurning)
		{
			...
			Instantiate(burnParticle, leftEngine.position, transform.rotation);
			Instantiate(burnParticle, rightEngine.position, transform.rotation);
